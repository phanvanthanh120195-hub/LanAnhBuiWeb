...
const PricingDesignThinking = () => {
    ...
    const loadTuitions = async () => {
        ...
        try {
            ...
            if (data) {
                // Filter by designThinking course
                const filtered = (data as Tuition[]).filter(
                    (t) => t.courseType === "designThinking"
                );
    ...
    const courseData = {
        title: currentTuition.title || "Tư Duy Thiết Kế: Sáng Tạo và Đổi Mới",
        price: calculateDiscountedPrice(
            currentTuition.originalPrice,
            currentTuition.discountPercent
        ),
        features: currentTuition.contentList || ["Phát triển tư duy sáng tạo trong thiết kế"],
        description: currentTuition.description ||
            "Khóa học giúp bạn phát triển tư duy thiết kế, từ việc tìm hiểu nhu cầu người dùng đến việc tạo ra các giải pháp sáng tạo và hiệu quả.",
    };
                            ...
                            <div className="aspect-[3/4] max-w-sm mx-auto bg-gradient-to-b from-background to-cream rounded-sm flex items-center justify-center border border-border/20 overflow-hidden">
                                {currentTuition?.imageUrl ? (
                                    ...
                                    <div className="text-center p-8">
                                        ...
                                        <p className="mt-4 font-heading text-sm italic text-muted-foreground">Design Thinking</p>
                                    </div>
                                )}
                            </div>
                        </div>
...
export default PricingDesignThinking;
